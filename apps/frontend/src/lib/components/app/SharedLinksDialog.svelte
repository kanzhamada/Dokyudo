<script lang="ts">
	...
	import { Check, Copy, ExternalLink, Link2, RefreshCw, Trash2 } from 'lucide-svelte';
	import { toast } from 'svelte-sonner';
	import { deleteShare, listAllShares } from '$lib/api/rag';
	...
	>
		...
		<div class="px-5 py-4">
			<div class="mb-3 flex items-center justify-between gap-3">
				...
				<button
					type="button"
					class="inline-flex items-center gap-1.5 rounded-md px-2 py-1 text-[11px] text-white/45 transition-colors hover:bg-white/[0.07] hover:text-white disabled:pointer-events-none disabled:opacity-40"
					disabled={isLoading}
					onclick={() => loadShares()}
				>
			...
			{:else}
				<div class="divide-y divide-white/[0.09] border-y border-white/[0.09]">
					{#each shares as share (share.code)}
						<div class="flex items-center gap-3 py-3">
							...
							<div class="flex shrink-0 items-center gap-0.5">
								<button
									...
									class="flex size-8 items-center justify-center rounded-md text-white/40 transition-colors hover:bg-white/[0.08] hover:text-white"
								...
								<button
									type="button"
									class="flex size-8 items-center justify-center rounded-md text-white/35 transition-colors hover:bg-red-500/[0.1] hover:text-red-300"
