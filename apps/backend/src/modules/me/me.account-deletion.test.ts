...
describe("MeService (account deletion)", () => {
  ...
  const cleanup = async () => {
    await db
      ...
      .where(eq(accountDeletionJobs.userId, testUserId));
    await db
      .delete(activityLogs)
      .where(eq(activityLogs.tenantId, testTenantId));
    await db
      .delete(paymentTransactions)
      .where(eq(paymentTransactions.tenantId, testTenantId));
    await db
  ...
  describe("requestAccountDeletion", () => {
    it("creates a pending job and flips user+tenant to deletion_pending", async () => {
      await db
        .insert(tenants)
        .values({ id: testTenantId, name: "Delete Request Tenant" });
      await db
        .insert(users)
        .values({ id: testUserId, tenantId: testTenantId, email: testEmail });
      await db
        .insert(tenantSubscriptions)
        .values({ tenantId: testTenantId, tier: "FREE" });
      ...
      try {
        ...
        const [job] = await db
          ...
          .where(eq(accountDeletionJobs.id, result.jobId));
    ...
    it("is idempotent — a second request returns the same job", async () => {
      ...
      try {
        const first = await MeService.requestAccountDeletion({
          userId: testUserId,
          tenantId: testTenantId,
        });
        const second = await MeService.requestAccountDeletion({
          userId: testUserId,
          tenantId: testTenantId,
        });
  ...
  describe("processJob (async purge)", () => {
    it("purges operational data, anonymizes, and retains payment + activity history", async () => {
      await db
        .insert(tenants)
        .values({ id: testTenantId, name: "Purge Tenant" });
      await db
        .insert(users)
        .values({ id: testUserId, tenantId: testTenantId, email: testEmail });
      await db
      ...
      await db
        ...
        .values({ id: convId, tenantId: testTenantId, title: "Secret Chat" });
        ...
        assertEquals(
          (
            await db
              .select()
              .from(chatShares)
              .where(eq(chatShares.code, shareCode))
          ).length,
          0,
        );
        assertEquals(
          (
            await db
              .select()
              .from(tenantKeys)
              .where(eq(tenantKeys.tenantId, testTenantId))
          ).length,
          0,
        );
    ...
    it("allows re-registration with the same email after the old account is purged", async () => {
      ...
      try {
        ...
        await db
          .insert(tenants)
          .values({ id: newTenantId, name: "New Tenant" });
        await db
          .insert(users)
          .values({ id: newUserId, tenantId: newTenantId, email: testEmail });
    ...
    it("is idempotent — re-running a completed job is a no-op", async () => {
      await db
        .insert(tenants)
        .values({ id: testTenantId, name: "Idempotent Tenant" });
      await db
        .insert(users)
        .values({ id: testUserId, tenantId: testTenantId, email: testEmail });
      await db
  ...
  describe("isUserActive (login / OAuth guard)", () => {
    it("returns true only for an active account", async () => {
      await db
        .insert(tenants)
        .values({ id: testTenantId, name: "Active Tenant" });
      await db
        .insert(users)
        .values({ id: testUserId, tenantId: testTenantId, email: testEmail });

      try {
        assertEquals(await MeService.isUserActive(testUserId), true);
      } finally {
        await cleanup();
      }
    });
    ...
    it("returns false for a deletion_pending account", async () => {
      ...
      try {
        assertEquals(await MeService.isUserActive(testUserId), false);
      } finally {
        await cleanup();
      }
    });
    ...
    it("returns false for a non-existent user", async () => {
      assertEquals(await MeService.isUserActive(crypto.randomUUID()), false);
    });
  });
  ...
  describe("purge billing cleanup (checkout+delete race)", () => {
    it("cancels ALL active subscriptions for the customer, not just the stored id", async () => {
      await db
        .insert(tenants)
        .values({ id: testTenantId, name: "Race Tenant" });
      await db
        .insert(users)
        .values({ id: testUserId, tenantId: testTenantId, email: testEmail });
      ...
      try {
        ...
        const cancelledIds = cancelStub.calls.map(
