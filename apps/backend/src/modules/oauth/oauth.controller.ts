...
import { setSessionCookies } from "../../config/cookie.ts";
...
async function processOAuthCallback(c: Context, provider: "google" | "github") {
  ...
  if (errorParam) {
    ...
    return c.redirect(
      `${frontendUrl}/login?oauth_error=${errorMsg}`,
    );
  }
  ...
  try {
    ...
    const frontendUrl = getEnv("FRONTEND_URL");
    return c.redirect(`${frontendUrl}/app/chat`);
  } catch (error: any) {
    if (logContext) {
    ...
    // Redirect to the login page with a readable error instead of
    // returning JSON (the user is in a browser redirect flow).
    const frontendUrl = getEnv("FRONTEND_URL");
    const errorMsg = encodeURIComponent(
      error.message || "OAuth authentication failed",
    );
    return c.redirect(`${frontendUrl}/login?oauth_error=${errorMsg}`);
  }
}
