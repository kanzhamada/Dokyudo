...
import { MeService } from "../me/me.service.ts";
...
export class OAuthService {
  ...
  static async initiateOAuth(
    ...
    const { data, error } = await supabase.auth.signInWithOAuth({
      ...
      options: {
        ...
        redirectTo: `${getEnv("API_URL")}/api/oauth/${params.provider}/callback`,
  ...
  static async handleOAuthCallback(
    ...
    let exchangeResult;
    try {
      exchangeResult = await supabase.auth.exchangeCodeForSession(params.code);
      ...
      try {
        ...
        if (params.logContext) {
          params.logContext.authWarning =
            "Failed to revoke unverified OAuth session: " + err.message;
        }
      }
    ...
    if (!(await MeService.isUserActive(user.id))) {
      try {
        await getSupabaseAdmin().auth.admin.signOut(
          session.access_token,
          "global",
    ...
    const avatarUrl = user.identities?.[0]?.identity_data?.avatar_url as
      string | undefined;
    ...
    if (tenantId) {
      ...
      if (params.logContext) {
        params.logContext.activityLogWarning = `Could not log activity for OAuth user ${user.id}: user record not found in public.users`;
      }
    }
  ...
  private static async resolveUserTenantId(params: {
    ...
    const lookupTenantId = async (): Promise<string | null> => {
      ...
      const [byId] = await db
        ...
        .from(users)
        .where(
          and(eq(users.id, params.userId), eq(users.deletionStatus, "active")),
        );
    ...
    try {
      ...
      throw new AppError({
        code: "INTERNAL_ERROR",
        message: "Failed to provision tenant for OAuth user. Please try again.",
        status: 500,
      });
    }
  }
