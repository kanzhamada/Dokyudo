  ...
  describe("POST /api/auth/login", () => {
    it("positive: successfully logs in user", async () => {
      ...
      if (res.status === 200) {
        ...
        // Tokens are delivered via httpOnly cookies — read the access
        // token from the Set-Cookie header for downstream tests.
        const setCookie = res.headers.get("set-cookie") ?? "";
        const match = setCookie.match(/dokyudo_access_token=([^;]+)/);
        validAccessToken = match ? decodeURIComponent(match[1]) : "";
      } else {
        console.warn(
