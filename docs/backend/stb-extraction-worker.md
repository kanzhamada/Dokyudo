...
2. **Konversi DOCX/TXT/MD → PDF (jika diperlukan)**: Untuk `.docx`/`.doc`/`.txt`/`.md`, worker menjalankan LibreOffice headless (`soffice --headless --convert-to pdf`) dengan timeout 120 detik dan profil terisolasi per job. PDF hasil konversi di-*upload* kembali ke MinIO sebagai `{tenant}/{docId}.pdf` — inilah yang dipakai frontend untuk viewer. **`libreoffice-math` wajib terpasang**: tanpa komponen ini rumus OMML (insert equation di Word) diimpor sebagai objek kosong dan hilang dari PDF. **Markdown tidak bisa dikonversi langsung**: LibreOffice membuka `.md` sebagai teks polos (`#`, `**`, pipa tabel tampil mentah), jadi `.md` dirender dulu ke HTML ber-CSS (via `python-markdown`, `services/markdown_extractor.py`) lalu HTML tersebut yang dikonversi ke PDF.
3. **Extraction & Chunking**:
   ...
   - **Markdown**: teks chunk diekstrak dari **HTML hasil render** (strip tag + unescape entity, blok `<style>/<script>` dibuang) — markdown mentah tidak pernah di-chunk langsung, sehingga noise sintaks (`#`, `**`, backtick) tidak mengotori embedding.
...
sequenceDiagram
    ...
    alt Ekstensi .docx / .doc / .txt / .md
        alt Ekstensi .md
            API->>Disk: python-markdown -> HTML + CSS
        end
        ...
        alt Ekstensi .docx
            ...
            API->>Disk: Strip tag dari HTML hasil render
        end
...
| `apps/stb-worker/services/txt_extractor.py` | Baca .txt dengan deteksi encoding (utf-8-sig, utf-16, utf-8, cp1252) + tolak file biner (`decode_text_bytes` dipakai juga oleh markdown) |
| `apps/stb-worker/services/markdown_extractor.py` | Render .md → HTML ber-CSS (python-markdown, ekstensi tables + fenced_code) + ekstraksi teks bersih dari HTML |
