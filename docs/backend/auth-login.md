...
6. **Idempotent Logout:** If a user calls `/api/auth/logout` with an already expired or invalid JWT, Supabase's `admin.signOut()` returns an error. Instead of surfacing this as a `500 INTERNAL_ERROR`, the backend intercepts it and treats it as a success. Since the end goal (invalidating the session) is already achieved, this provides a clean, idempotent UX for the frontend to clear its local state without erroring.
