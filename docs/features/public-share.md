...
- Di atas user pill (di luar bubble) ditampilkan kartu attachment **statis** dari `turn.attachments` — kotak 1:1 rounded dengan ikon dokumen besar, judul di bawahnya (truncate `…` saat panjang), dan ekstensi file — tanpa handler klik: judul terlihat, dokumen tidak bisa dibuka, tanpa error.
